...
var TreeList = /** @class */ (function (_super) {
    ...
    TreeList.prototype.render = function () {
        ...
        var columnsWithColGroup = this.extendedColumn.filter(function (c) { return c.children.length === 0; });
        ...
        var dataRow = function (item, inEdit, rowId, isRowExpanded, rowDataIndex, selectedValue) { return columnsWithColGroup.map(function (column, columnIndex) {
            ...
            if (inEdit && column.editCell) {
                return (React.createElement(column.editCell, __assign({ key: columnIndex }, cellProps, { onChange: _this.itemChange })));
            }
            if (column.cell) {
                return React.createElement(column.cell, __assign({ key: columnIndex }, cellProps));
            }
            return React.createElement(TreeListCell_1.TreeListCell, __assign({ key: columnIndex }, cellProps));
        }); };
        ...
        var body = flattedData.length > 0 && flattedData.map(function (item, rowIndex) {
            ...
            React.createElement("td", { colSpan: columnsWithColGroup.length }, this.props.noRecords || React.createElement(TreeListNoRecords_1.TreeListNoRecords, null))));
        var sorted = function (field) {
            return _this.props.sort && _this.props.sort.some(function (descriptor) { return descriptor.field === field; });
        };
        var colGroups = (React.createElement("colgroup", { ref: function (c) { _this.columnResize.colGroupMain = c; }, role: 'presentation' }, columnsWithColGroup.map(function (column, index) { return (React.createElement("col", { key: index.toString(), className: sorted(column.field) ? 'k-sorted' : undefined, style: column.width !== undefined ? { width: column.width } : undefined })); })));
        ...
        return (React.createElement(kendo_react_data_tools_1.TableKeyboardNavigationContext.Provider, { value: this.contextStateRef.current },
            React.createElement("div", __assign({ style: this.props.style, className: kendo_react_common_1.classNames('k-widget k-grid', this.props.className, { 'k-treelist-scrollable': scrollable !== 'none' }), ref: function (e) { return _this.element = e; }, onScroll: virtualScroll ? this.handleOnScroll : undefined, onKeyDown: this.onKeyDown, onFocus: this.onFocus, "aria-rowcount": total, "aria-colcount": columnsWithColGroup.length, role: 'treegrid' }, kendo_react_data_tools_1.tableKeyboardNavigationScopeAttributes),
                this.props.toolbar,
